...
import com.unistack.tamboo.message.kafka.bean.ClusterNode;
...
public class TopicAdmin implements AutoCloseable {
    ...
    public static  Logger log = LoggerFactory.getLogger(TopicAdmin.class);

    public static  int OPERATION_TIMEOUT = 60 * 1000;

    public static  String PRINCIPAL_PREFIX = "User:";
     ...
     * @return
     */
    public boolean createTopicForAcl(String topic, String user, int code) {
        if (StringUtils.isBlank(topic)) {
            return false;
        }
        createGroupForAcl();//默认创建所有group都可以访问
        String topicName = createTopicForAcls(topic, code, null, null, user);
    ...
    public Set<ClusterNode> getBrokerList() {
        ClusterDescription description = listCluster();
        return description.getClusterNode();
    }
    ...
    public String createTopicForAcls(String topic, int code, AclPermissionType permissionType, String... users) {
        ...
        if (StringUtils.isNotBlank(topic)) {
            for (String user : users) {
                if (code == 3) {
                    Resource resource = new Resource(ResourceType.TOPIC, topic);
                    AccessControlEntry accessControlEntry = new AccessControlEntry(PRINCIPAL_PREFIX + user, "*", AclOperation.WRITE, permissionType == null ? AclPermissionType.ALLOW : permissionType);
                ...
                if (code == 2) {
                    Resource resource = new Resource(ResourceType.TOPIC, topic);
                    AccessControlEntry accessControlEntry2 = new AccessControlEntry(PRINCIPAL_PREFIX + user, "*",
                            AclOperation.READ, permissionType == null ? AclPermissionType.ALLOW : permissionType);
                    AclBinding aclBinding = new AclBinding(resource, accessControlEntry2);
                    aclBindings.add(aclBinding);
                }
                ...
                if (code == 1) {
                    ...
                    AccessControlEntry accessControlEntry1 = new AccessControlEntry(PRINCIPAL_PREFIX + user, "*",
                            AclOperation.WRITE, permissionType == null ? AclPermissionType.ALLOW : permissionType);
                    AclBinding aclBinding1 = new AclBinding(resource1, accessControlEntry1);
    ...
    public static void main(String[] args) {
        ...
        map.put(SaslConfigs.SASL_JAAS_CONFIG, jaasConfigProperty("PLAIN", "admin", "admin123").value());
        map.put(AdminClientConfig.SECURITY_PROTOCOL_CONFIG, SecurityProtocol.SASL_PLAINTEXT.name);
        map.put(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, "192.168.1.110:9093,192.168.1.111:9093,192.168.1.112:9093");
        TopicAdmin topicAdmin = new TopicAdmin(map);
        System.out.println(topicAdmin.getHostToIdInfo());
    }
